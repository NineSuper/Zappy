...
/*   Updated: 2025/05/27 12:13:43 by tde-los-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
fn draw_ui(
    ...
    let (logs_display, scroll_info) = if total_logs == 0 {
        ...
        let scroll_info = if scroll_offset > 0 {
            format!(
                " (↑{}/{})",
                scroll_offset,
                total_logs.saturating_sub(available_lines)
            )
        } else {
            String::new()
        };
    ...
    let status_left = Paragraph::new(Text::from(vec![
        ...
        Line::from(Span::raw(format!(
            "👥 Clients: {}",
            app_state.server.clients.len()
        ))),
        Line::from(Span::raw(format!("🗺️ Map: {}x{}", map_width, map_height))),
        Line::from(Span::raw(format!(
            "⏱️ Tickrate: {}/s",
            app_state.settings.time_unit
        ))),
    ]))
    ...
    let network_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                format_bytes(metrics.network_rx_bytes),
