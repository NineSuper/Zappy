...
lazy_static::lazy_static! {
	static ref SYSTEM_METRICS: Arc<Mutex<SystemMetrics>> = Arc::new(Mutex::new(SystemMetrics {
		cpu_usage: 0.0,
		memory_usage: 0.0,
		memory_used: 0,
		memory_total: 0,
	...
	static ref SYSTEM_MONITOR: Arc<Mutex<Option<System>>> = Arc::new(Mutex::new(None));
	static ref LAST_NETWORK_STATS: Arc<Mutex<(u64, u64, Instant)>> = Arc::new(Mutex::new((0, 0, Instant::now())));
}
...
pub fn display_init()
{
	...
	execute!(stdout, EnterAlternateScreen, EnableMouseCapture)
		.expect("EnterAlternateScreen failed");
...
pub fn display_cleanup()
{
	unsafe {
		if let Some(mut display) = DISPLAY.take()
		{
			...
			execute!(display.terminal.backend_mut(), LeaveAlternateScreen, DisableMouseCapture)
...
lazy_static! {
	static ref NETWORK_MONITOR: Mutex<Option<Networks>> = Mutex::new(None);
}

// Fonction pour mettre à jour les métriques système
fn update_system_metrics()
{
	let mut system_opt = SYSTEM_MONITOR.lock().unwrap();

	if system_opt.is_none()
	{
		*system_opt = Some(System::new_all());
	}

	if let Some(ref mut system) = system_opt.as_mut()
	{
		...
		// Memory Usage
		metrics.memory_used = system.used_memory();
		metrics.memory_total = system.total_memory();
		metrics.memory_usage = if metrics.memory_total > 0
		{
				...
				unsafe {
					if let Some(display) = &mut DISPLAY
					{
						...
						display.scroll_offset =
							(display.scroll_offset + 10).min(logs.len().saturating_sub(1));
					}
				},
				KeyCode::PageDown =>
				unsafe {
...
fn parse_ansi_to_segments(text: &str) -> Vec<StyledSegment>
{
	...
	while let Some(ch) = chars.next()
	{
		if ch == '\x1b' && chars.peek() == Some(&'[')
		{
			...
			// Parser le code ANSI
			let codes: Vec<u8> = ansi_code.split(';').filter_map(|s| s.parse().ok()).collect();
...
fn get_percentage_color(percentage: f32) -> ratatui::style::Color
{
...
)
{
	...
	let chunks = Layout::default()
		...
		.constraints([
			...
			Constraint::Length(6), // Status (augmenté pour les barres)
	...
	else
	{
		...
		let displayed_logs: Vec<Line> =
			logs[start_index..end_index].iter().map(|line| ansi_to_ratatui_line(line)).collect();
	...
	// Block système
	let system_block = Block::default().borders(Borders::ALL).title("💻 Système");
	...
	let network_lines = vec![
		Line::from(vec![
			...
			Span::styled(
				format_bytes(metrics.network_rx_rate as u64),
		...
		Line::from(vec![
			...
			Span::styled(
				format_bytes(metrics.network_tx_rate as u64),
				Style::default().fg(Color::White).add_modifier(Modifier::BOLD),
			),
			Span::styled("/s", Style::default().fg(Color::Gray)),
		]),
		...
		Line::from(vec![
			...
			Span::styled(
				format_bytes(metrics.network_tx_bytes),
