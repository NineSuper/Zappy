...
/*   Updated: 2025/07/01 11:31:14 by tde-los-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
impl Team
{
	...
	{
		let players_json: Vec<String> =
			self.players.iter().map(|player| player.get_json()).collect();
...
{
	let team_jsons: Vec<String> = teams.iter().map(|team| team.get_json()).collect();
